...
const App = () => {
  ...
  const hook = () => {
    communicationService.getAll().then((initialPeople) => {
  ...
  const addPerson = (event) => {
    ...
    if ((newName.length === 0) | (newNumber.length === 0)) {
      ...
      if (
        window.confirm(
          `${
            persons.filter((person) => person.name === newName)[0].name
          } has already been added to the phonebook. Would you like to replace the old number with the new one?`
        )
      ) {
